...
from metrics.bertscore.score import score as bertscore
from .knn import ratio_margin_align
import torch
import pulp
import logging
import numpy as np
...
class SentSimScorer:
    ...
    def _embed(self, source_sents, target_sents):
        return (
            self.sent_model.encode(source_sents, convert_to_tensor=True, batch_size=self.embedding_batch_size).cpu(),
            self.sent_model.encode(target_sents, convert_to_tensor=True, batch_size=self.embedding_batch_size).cpu())
    ...
    def getBertScore(self, sents1, sents2, model):
        P, R, F1 = bertscore(sents2, sents1, model_type=model, batch_size=self.embedding_batch_size, device=self.device)
        score = torch.tensor(F1)
